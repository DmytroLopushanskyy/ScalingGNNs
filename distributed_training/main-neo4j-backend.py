...
def test(
    ...
    def test_homo(batch):
        ...
        out = model(batch.features, batch.edge_index)[:batch.batch_size]
        y_pred = out.argmax(dim=-1)
        y_true = batch.label[:batch.batch_size]
        return y_pred, y_true
...
def train(
    ...
    def train_homo(batch):
        out = model(batch.features, batch.edge_index)[:batch.batch_size]
        loss = F.cross_entropy(out, batch.label[:batch.batch_size].long())
...
def run_proc(
    ...
    train_idx = torch.load(
        osp.join(
            ...
            f'partition{node_rank % 2}.pt',
        ))
    train_slice_start = math.floor(int(node_rank/2) * train_idx.shape[0] / (num_nodes/2))
    train_slice_end = math.ceil((int(node_rank/2) + 1) * train_idx.shape[0] / (num_nodes/2))
    train_idx = train_idx[train_slice_start:train_slice_end]
    print(f"train from {train_slice_start} to {train_slice_end}")
    ...
    test_slice_start = math.floor(int(node_rank/2) * test_idx.shape[0] / (num_nodes/2))
    test_slice_end = math.ceil((int(node_rank/2) + 1) * test_idx.shape[0] / (num_nodes/2))
    test_idx = test_idx[test_slice_start:test_slice_end]
    print(f"test from {test_slice_start} to {test_slice_end}")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--num_loader_threads',
        type=int,
        default=16,  # Change to 8 from 10
